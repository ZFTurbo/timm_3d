...
from timm_3d.layers import BatchNormAct3d, get_norm_act_layer, BlurPool3d, create_classifier
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        if aa_layer is None:
            stem_pool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        else:
        ...
        if deep_stem:
            ...
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv3d(in_chans, stem_chs_1, 3, stride=2, padding=1, bias=False)),
                ('norm0', norm_layer(stem_chs_1)),
                ('conv1', nn.Conv3d(stem_chs_1, stem_chs_2, 3, stride=1, padding=1, bias=False)),
                ('norm1', norm_layer(stem_chs_2)),
                ('conv2', nn.Conv3d(stem_chs_2, num_init_features, 3, stride=1, padding=1, bias=False)),
...
def densenetblur121d(pretrained=False, **kwargs) -> DenseNet:
    ...
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    """
    model_args = dict(growth_rate=32, block_config=(6, 12, 24, 16), stem_type='deep', aa_layer=BlurPool3d)
