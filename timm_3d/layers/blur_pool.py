...
class BlurPool3d(nn.Module):
    ...
    def __init__(self, channels, filt_size=3, stride=2) -> None:
        ...
        self.stride = stride
        self.padding = [get_padding(filt_size, stride, dilation=1)] * 6
        coeffs = torch.tensor((np.poly1d((0.5, 0.5)) ** (self.filt_size - 1)).coeffs.astype(np.float32))
        blur_filter = (coeffs[:, None, None] * coeffs[None, :, None] * coeffs[None, None, :])[None, None, :, :, :].repeat(self.channels, 1, 1, 1, 1)
