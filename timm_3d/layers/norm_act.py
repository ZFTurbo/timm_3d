...
class BatchNormAct3d(nn.BatchNorm3d):
    ...
    def forward(self, x):
        ...
        _assert(x.ndim == 5, f'expected 4D input (got {x.ndim}D input)')
...
def convert_sync_batchnorm(module, process_group=None):
    ...
    if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
        if isinstance(module, BatchNormAct3d):
...
def freeze_batch_norm_2d(module):
    ...
    """
    res = module
    if isinstance(module, (BatchNormAct3d, SyncBatchNormAct)):
...
def unfreeze_batch_norm_2d(module):
    ...
    if isinstance(module, FrozenBatchNormAct2d):
        res = BatchNormAct3d(module.num_features)
        if module.affine:
            res.weight.data = module.weight.data.clone().detach()
            res.bias.data = module.bias.data.clone().detach()
        res.running_mean.data = module.running_mean.data
...
class LayerNormAct3d(nn.LayerNorm):
    def __init__(
            self,
            num_channels,
            eps=1e-5,
        ...
        super(LayerNormAct3d, self).__init__(num_channels, eps=eps, elementwise_affine=affine)
        self.drop = drop_layer() if drop_layer is not None else nn.Identity()
        self.act = _create_act(act_layer, act_kwargs=act_kwargs, inplace=inplace, apply_act=apply_act)
        self._fast_norm = is_fast_norm()
